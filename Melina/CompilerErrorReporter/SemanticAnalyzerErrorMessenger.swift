struct SemanticAnalyzerErrorMessenger: ErrorMessenger {
    ...
    func message(for error: SemanticAnalyzerError) -> String {
        ...
        case .incompatibleAction(let action, let element):
            return "action `\(action.lexeme)` can't be applied to the element `\(element.lexeme)`"
        case .suiteNameCollision(let suite):
            return "suite with `\(suite.lexeme)` name is already declared`"
        case .scenarioNameCollision(let scenario):
            return "scenario with `\(scenario.lexeme)` name is already declared`"
        }
    }
}
